...
async function processImage(url, nameBase) {
  try {
    const tmpPath = path.join(publicDir, nameBase + path.extname(new URL(url).pathname).split('?')[0] || '.jpg');
...
async function main() {
  ...
  const files = [eventsFile, artistsFile];
  ...
  for (const file of files) {
    ...
    for (const url of urls) {
      if (url.startsWith('data:')) continue; // skip embedded base64 images for now
      try {
        ...
        if (res) {
          // Replace first occurrence of the url in the file with the optimized 800px webp path
          const newContent = content.replace(url, res.out800);
